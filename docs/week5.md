...
- Step 2: We reduce the stride by half (Stride = 4), meaning we add the values with IDs 0 and 4 and store the sum into ID 0 of the next array. Likewise we do the same for IDs 1–3 and 5–7. The results are stored into the next array with IDs 0–3. Again, we reduced the previous array by half: from 8 to 4 elements.
